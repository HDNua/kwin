...
def get_random_distorted_bottlenecks(
    ...
    for unused_i in range(how_many):
        ...
        ### [17-09-30 18:09] HANDY: 주석을 달았습니다.
        #
...
def add_input_distortions(flip_left_right, random_crop, random_scale,
    ...
    # [17-09-30 18:12] HANDY: 주석을 강화했습니다.
...
def main(_):
    ...
    ### [17-09-30 18:02] HANDY: 압축 파일을 다운로드한후 제거하는 코드인데 불필요한 것으로 보입니다.
...
def do_train():
    # [17-09-30 21:04] HANDY: 전역에서 함수로 옮기다가 FLAGS가 전역인 거 잊어먹고 시간 날렸네 ㅡㅡ
    global FLAGS

    image_dir = dataset.train_data_path()
    train_dir = dataset.TRAIN_DIR
    bottleneck_dir = dataset.bottleneck_dir()
    model_dir = dataset.model_dir()
    output_graph = dataset.output_graph()
    output_labels = dataset.output_labels()

    print("--image_dir      : [%s]" %image_dir)
    print("--train_dir      : [%s]" %train_dir)
    print("--bottleneck_dir : [%s]" %bottleneck_dir)
    print("--model_dir      : [%s]" %model_dir)
    print("--output_graph   : [%s]" %output_graph)
    print("--output_labels  : [%s]" %output_labels)

    # HANDY NOTE: --image-dir 옵션은 기본적으로 RESOURCE_DIR/image_rotator/test/wr 폴더를 가리킨다
    ...
    parser.add_argument(
        '--output_graph',
        type=str,
        default=output_graph,  # '/tmp/output_graph.pb',
        help='Where to save the trained graph.'
    )
    parser.add_argument(
        '--output_labels',
        type=str,
        default=output_labels,  # '/tmp/output_labels.txt',
    ...
    parser.add_argument(
        '--model_dir',
        type=str,
        default=model_dir,  # '/tmp/imagenet',
        help="""\
    ...
    parser.add_argument(
        '--bottleneck_dir',
        type=str,
        default=bottleneck_dir,  # '/tmp/bottleneck',
