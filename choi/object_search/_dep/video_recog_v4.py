...
white = cv2.imread(resource_path("white-640x480.jpg"))
if white is None:
...
# video = cv2.VideoCapture(resource_path("testing.mp4"))

# 레이블 파일을 가져오고(GFile) 모든 줄의 끝에 있는 캐리지 리턴을 제거합니다(rstrip).
label_lines = [line.rstrip() for line
               in tf.gfile.GFile(project_path("train/retrained_labels.txt"))]

# 그래프 파일로부터 그래프를 생성합니다.
with tf.gfile.FastGFile(project_path("train/retrained_graph.pb"), 'rb') as f:
